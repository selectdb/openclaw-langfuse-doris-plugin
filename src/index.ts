...
export const langfusePlugin = {
  ...
  activate(api: OpenClawPluginApi): void {
    ...
    if (!pluginConfig.publicKey) {
      api.logger.error("[Langfuse] Missing required configuration: 'publicKey' must be provided");
      return;
    }

    if (!pluginConfig.secretKey) {
      api.logger.error("[Langfuse] Missing required configuration: 'secretKey' must be provided");
      return;
    }
